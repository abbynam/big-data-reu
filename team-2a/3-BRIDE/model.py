...
class ImprovedLSTM2024(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        # Initializing hidden state for first input with zeros
        h0 = torch.zeros(self.num_lstm_layers, x.size(0), self.hidden_state_size).to(x.device).requires_grad_()
        # Initializing cell state for first input with zeros
        c0 = torch.zeros(self.num_lstm_layers, x.size(0), self.hidden_state_size).to(x.device).requires_grad_()
...
class GAT(pl.LightningModule):
    def __init__(self, input_size, num_classes, hidden_layers, activation, lr, lr_step, lr_gam, penalty, dropout=0, l2=0, heads=1, mlp_nodes=256):
        super().__init__()
        self.save_hyperparameters()
        ...
        # FCN model
        self.output_layer = nn.Linear(mlp_nodes, num_classes)
        #self.mlp_aggregation = MLPAggregation(self.hidden_layers[-1] * self.heads, mlp_nodes, 3, num_layers=2)
        # regular model
        in_layer = input_size
        self.model_layers = nn.ModuleList()
        for num in self.hidden_layers:
            layer = GATv2Conv(in_layer, num, heads=heads, concat=True)
            self.residuals.append(nn.Identity() if in_layer == num else nn.Linear(in_layer, num))
            self.model_layers.append(layer)
            in_layer = num * heads



    def forward(self, input):  # , dropout_on=True
        ...
        #x = self.mlp_aggregation(x, index=batch)
        x = self.output_layer(x)
    ...
    def loss_fn(self, logits, y):
        ...
        arr = torch.argmax(logits, dim=1)
        encoded_arr = torch.zeros((arr.size(dim=0), 13), dtype=torch.int).to(logits.device)
        encoded_arr[torch.arange(arr.size(dim=0)), arr] = 1
        pair_wise_x = encoded_arr
    ...
    def validation_step(self, batch, batch_idx):
        y = batch.y
        logits = self(batch)  # , dropout_on=False
        loss = self.loss_fn(logits, y)
        preds = torch.argmax(logits, dim=1)
        self.valid_acc(preds, y)
        self.log("valid_acc", self.valid_acc, on_step=True, on_epoch=True, prog_bar=True, sync_dist=True)
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        # see predict.py, was running into an Attribute no Flatten error [resolved]
        logits = self(batch)  # , dropout_on=False
        preds = torch.argmax(logits, dim=1)
        return preds
