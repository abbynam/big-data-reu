...
param_grid = {
    ...
    'lr': [0.001, 0.0005, 0.0001],  # change as needed
    'lr_step':[1000],
    'lr_gam':[0.1], #change as needed 
    'dropout': [0.05, 0.15, 0.3],  # change numbers and add to rows as necessary 
    'data_name': ['patient_2025'],  # change dataset names as needed
    'l2':[0.0000001]
}

# === NEEDED PATHS ===
Template_path = '../../grid_search/impr_LSTM24.yaml' #grid_search folder is stored in base
...
for i, params in enumerate(grid):
    ...
    tc['fit']['ckpt_freq'] = 400 #CHANGE IF NEEDED
    tc['model']['indim'] = 15
    tc['model']['outdim'] = 13
    tc['model']['num_linears'] = params['num_linears']
    tc['model']['neurons_per_hidden'] = params['neurons_per_hidden']   
    tc['model']['input_neurons'] = 128
    tc['model']['num_lstm_layers'] = params['num_lstm_layers']
    tc['model']['hidden_state_size'] = params['hidden_state_size']
    ...
    tc['model']['penalty'] = 1 #change if needed
    tc['model']['optimizer'] = 'adam'   
    tc['model']['one_activation'] = False 
    tc['model']['custom_loss'] = True
    tc['model']['bias'] = True 
 # write config YAML
