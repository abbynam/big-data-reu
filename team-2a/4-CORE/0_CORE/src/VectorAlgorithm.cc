...
Image2D VectorAlgorithm::getImagePlane(size_t dimension, float depth) const{
    ...
    if(0 == dimension){
        ...
        err_msg <<"Invalid dimension " << dimension << "for VectorAlgorithm:getImagePlane." << endl;
...
Image3D VectorAlgorithm::getImageVolume(size_t dimension) const{
...
string VectorAlgorithm::get_event_record_(long event_num) const{
    stringstream ss;
    ss.precision(7);
...
string VectorAlgorithm::getConicInformationAsString() const{
    ...
    cout<<"Ading records to string stream . . ."<<endl;
...
//std::array<PGVector3, 8> VectorAlgorithm::get_octant_centers(const PGVector3 &center, const double length){
std::vector<PGVector3> VectorAlgorithm::get_octant_centers(const PGVector3 &center, const double length){
...
vector<PGVector3> VectorAlgorithm::get_intercepts(const ConicSection &cs, const PGVector3 &center, float length, float intercept_dca){
...
void VectorAlgorithm::set_intercepts(const ConicSection &cs, const PGVector3 &center, float length, float intercept_dca){
...
//                float weight = pow(copy.y*this->SAD_, cone_length_correction_)*1.0E-3; //normalize Y dimension to 100 cm
//                float weight = pca.getDistanceToPoint(cs.getComptonScatter().getConeApex())*this->SAD_;
                float weight = pca.getDistanceToPoint(cs.getComptonScatter().getConeApex());
//                float weight = 1.0;
                //float weight = 1000.0;
...
void VectorAlgorithm::populate_density_matrix(const vector<ConicSection> &conics){

    auto num_cones = conics.size();
...
void VectorAlgorithm::run(){
    cout<<"VectorAlgorithm::run() . . ."<<endl;
    populate_density_matrix(conic_sections_);
    cout<<"VectorAlgorithm::run() complete . . ."<<endl;
    return;
}


void VectorAlgorithm::setConicSections(const vector<ConicSection> &conic_sections){
    conic_sections_.clear();
    for(size_t i = 0; i<conic_sections.size(); ++i){
        conic_sections_.push_back(conic_sections[i]);
    }
    cout<<"Copied "<<conic_sections.size() <<" conic sections to the VectorAlgorithm  . . ."<<endl;
}
