...
#include "../include/PointAlgorithm.h"

using namespace std;
using namespace prompt_gamma_reconstruction;

Image2D PointAlgorithm::getImagePlane(size_t dimension, float depth) const{
    ...
    if(0 == dimension){
        ...
        err_msg <<"Invalid dimension " << dimension << "for PointAlgorithm:getImagePlane." << endl;
...
Image3D PointAlgorithm::getImageVolume(size_t dimension) const{
    Image3D I(vector<float>{-3.0,-3.0,-3.0}, vector<float>{0.0,0.0,0.0}, vector<size_t>{0,0,0});

    return I;
}

void PointAlgorithm::setConicSections(const vector<ConicSection> &conic_sections){
    conic_sections_ = conic_sections;
}

void PointAlgorithm::run(){
...
//    double new_prior = 1.0; //Gaussian likelihood weight of random test point
    ...
    long number_tries;
    long long i = 0;
    for (; i < (long long)number_of_iterations_; i++) {
        changeCount = 0;

        long long index = i;
#pragma omp parallel for reduction(+:changeCount), \
                private(index, oldEventPos, randomPosition, oldDensity, newDensity, event_weight, number_tries ) num_threads(number_of_threads_)
        for(size_t jEventNum=0; jEventNum< num_cones; ++jEventNum){
            ...
            oldDensity = density_estimator_ptr_->getDensity(oldEventPos) - event_weight;
...
//            newDensity = density_estimator_ptr_->getDensity(randomPosition)*new_prior;
            ...
            auto ratio = randomPosition.getDistanceToPoint(conic_sections_[jEventNum].getComptonScatter().getConeApex())/
                         oldEventPos.getDistanceToPoint(conic_sections_[jEventNum].getComptonScatter().getConeApex());
            auto distance_correction = pow(ratio, inverse_square_parameter_);


//            cout<<"Distance corr "<<distance_correction<<"\n";

            //Acquire the mutex and make the change if the new density
            // is greater than the old density times a random raised to
            // power temperature.
            if( newDensity*distance_correction >= pow(rand, temperature_)*( oldDensity )){
                conic_sections_[jEventNum].setLikelyOrigin(randomPosition);

                ///DSM lock moved to the density estimator itself
                //#pragma omp critical
                {
        ...
        if ( ( i < 100 &&  (i+1) % 10 == 0) ||
            ...
            printf("Iteration: %ld, time %ld, Number of Position Changes: %ld, ratio: %.3f",
                   (long)(i + 1), get_running_time(), changeCount, static_cast<float>(changeCount)/static_cast<float>(num_cones) );
            cout<<endl;//print it out now!

        }
    }
    printf("--- Total Iterations: %ld, time %ld, Number of Position Changes: %zu, %lu, ratio: %.3f\n",
           (long)number_of_iterations_, get_running_time(), changeCount, num_cones, (float)changeCount/(float)(num_cones));
}


void PointAlgorithm::populate_density_matrix_(const vector<ConicSection> &conicSections, DensityEstimator &density_estimator){
...
string PointAlgorithm::getConicInformationAsString() const{
    stringstream ss;
