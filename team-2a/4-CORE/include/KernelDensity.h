#ifndef KERNEL_DENSITY_H_
#define KERNEL_DENSITY_H_
...
namespace prompt_gamma_reconstruction {
    ...
    class KernelDensity : public DensityEstimator {
    ...
    public:
        ...
        KernelDensity() :
            ...
            cout << "KernelDensity::KernelDensity(empty constructor):" << counts_vector_.size() << endl;
        };


        KernelDensity(const KernelDensity &other) : x_min_(-1.0), x_max_(1.0), x_bins_(1), x_bin_size_reciprocal_(1.0),
                                                    y_min_(-1.0), y_max_(1.0), y_bins_(1), y_bin_size_reciprocal_(1.0),
                                                    z_min_(-1.0), z_max_(1.0), z_bins_(1), z_bin_size_reciprocal_(1.0),
        ...
        ~KernelDensity() {
            if (-1 != bad_bin_) {
                cerr << "WARNING: the value for KernelDensity.bad_bin = " << bad_bin_ << ". It should be equal to -1.\n"
        ...
        inline float getDensity(const PGVector3 &pos) const {
            ...
            if (counts_vector_.size() <= bin_number) {
                stringstream ss;
                ss << "ERROR in KernelDensity::getDensity(" << pos.x << "," << pos.y << "," << pos.z <<
        ...
        inline float &operator[](const PGVector3 &pos) {
            size_t bin_number = getBinNumber_(pos);
            if (bin_number < 0) {
                stringstream ss;
                ss << "ERROR in KernelDensity::operator[]: point(" << pos.x << "," << pos.y << "," << pos.z <<
                ") returned bin "
                << bin_number << ".\n";
                cout << ss.str() << endl;
                //print();
        ...
        inline void updateMatrix(const PGVector3 &oldPos, const PGVector3 &newPos, const float &weight) {
            ...
            //causes deadlock -> #pragma omp critical
             {
                 counts_vector_[getBinNumber_(oldPos)] -= weight;
                 counts_vector_[getBinNumber_(newPos)] += weight;
             }
        };

        KernelDensity &operator=(float rhs) {
            counts_vector_ = rhs;
            return *this;
        }


        KernelDensity &operator=(const KernelDensity &rhs) {
            make_copy(rhs);
            return *this;
        }
        ...
        KernelDensity &operator+=(const KernelDensity &rhs) {
            if (counts_vector_.size() != rhs.counts_vector_.size()) {
                stringstream ss;
                ss << "ERROR in KernelDensity &operator+= :\n";
    ...
    private:
        ...
        void make_copy(const KernelDensity &model) {
...
//            size_t bx = static_cast<int>((pos.x - x_min_) * x_bin_size_reciprocal_);
//            size_t by = static_cast<int>((pos.y - y_min_) * y_bin_size_reciprocal_);
//            size_t bz = static_cast<int>((pos.z - z_min_) * z_bin_size_reciprocal_);
//            size_t bin_number = bx + x_bins_*by + x_bins_*y_bins_*bz;
            ...
            bin_number += y_bins_ * x_bins_ * static_cast<int>((pos.z - z_min_) * z_bin_size_reciprocal_);
