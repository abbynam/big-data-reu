...
class pixel:
    ...
    '''
    def __init__(self,module=0,crystal=0,xInd=0,zInd=0,edep=0,rawX=0,rawY=0,rawZ=0,time=0,peakTrack=0,pulseShape=np.ones((501)),eScl=1,eOut=0,pulseLength=5E-6,tShapeThresh=0.001,trigThresh=0.05,resTime = 1.5E-6):
...
def returnPixelLocs(events,numPixX=11,numPixZ=11):
    ...
    for iii in range(16):
        ...
        c = getCrystal(q)
        for jjj in range(4):
            print(f"Crystal {jjj}: {(c==jjj).sum()} events")
...
def makePixFromCloud(eDepPix,pixXInd,pixZInd,xC,zC,xRef,zRef,yTest,mInd,cInd,t,tCurrent,gEnes,depths,muList,cList,aList,bList,dList,p1Anode=295,p2Anode=327.5,crysThick1=10,crysThick2=15):
    '''
    ...
    if len(pixXInd) == 1 and len(pixZInd) == 1: #single pixel
        ...
        pixNew = pixel(module=mm,crystal=cc,xInd=xx,zInd=zz,edep=ed,rawX=rX,rawY=rY,rawZ=rZ,time=tCurrent,eScl=escl,pulseShape=l)
        pList.append(pixNew)
...
def getCrystal(eventsRaw):
    ...
    if events.shape[1]==4:
        ...
        quad1[crysTR1] = 0 # crystals are named in order of top right, top left, bottom left and bottom right
        quad1[crysTL1] = 1
        quad1[crysBL1] = 2
        quad1[crysBR1] = 3
    ...
    while iii<totEvents:
        ...
        if nList[iii] == 0: #single
            ...
            if modList[mm.astype(int)].status <= 1:
                ...
                pOut = makePixFromCloud(eDepPix,pixXInd,pixZInd,xC,zC,xRef,zRef,rY,mm,cc,t,tCurrent,gEnes,depths,muList,cList,aList,bList,dList,plane1Anode,plane2Anode,crysThick1,crysThick2)    
