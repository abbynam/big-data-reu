...
def numToMetricString(string):
    ...
    ### EDIT for DEBUGGING 7/2/25
    print(num)
    ###

    ## EDIT PJIN MADE IN CODE on 7/2/25 BECAUSE NUM of FALSE TRIPLES = 0 (strange), causing num=0
    if num == 0:
        maxPower = 0
    else:
        maxPower = math.log(num)/math.log(10)
    ###
    ### REPLACING THIS BELOW - THIS REPLACEMENT MAY BE FUNDAMENTALLY INCORRECT
    ###maxPower = math.log(num)/math.log(10)
    ###
    if maxPower <= 3:
        s = "{:3d}".format(num)
...
def getAutoName(allFiles, allCounts, order, filename_pieces=None):
    ...
    # print(allCounts)
    # print(order)
    print(f"allCounts in getAutoName: {allCounts}") 
    print(f"order in getAutoName: {order}")
    filenames = []
    for key in order:
        tList = allFiles[key]
