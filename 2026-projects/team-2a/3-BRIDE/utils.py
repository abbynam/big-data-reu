...
class PGMLDataModule(pl.LightningDataModule):
    def __init__(self, train_data_path='./', test_data_path='./', batch_size:int=32, val_split:float=0.1, model_key=''):
        super().__init__()
    ...
    def setup(self, stage: str):
        ...
        if stage == "fit":
            ...
            if self.model_key == 'gat':
                edge_index = torch.tensor([[0,1,1,2,0,2],[1,0,2,1,2,0]])
                data_full = [DataPyG(x=torch.tensor(x), edge_index=edge_index, y=torch.tensor(y))
                        for x,y in zip(np.load(X_path).astype(np.float32), np.load(y_path))]
    ...
    def train_dataloader(self):
        if self.model_key == 'gat':
            return DataLoaderPyG(self.train, batch_size=self.batch_size)
        return DataLoader(self.train, batch_size=self.batch_size)
    

    def val_dataloader(self):
        if self.model_key == 'gat':
            return DataLoaderPyG(self.val, batch_size=self.batch_size)
        return DataLoader(self.val, batch_size=self.batch_size)


    def test_dataloader(self):
        if self.model_key == 'gat':
            return DataLoaderPyG(self.test, batch_size=self.batch_size)
        return DataLoader(self.test, batch_size=self.batch_size)


    def predict_dataloader(self):
        if self.model_key == 'gat':
            return DataLoaderPyG(self.test)
